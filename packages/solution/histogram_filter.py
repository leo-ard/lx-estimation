...
def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    ...
    left_dtick = alpha * left_encoder_ticks
    right_dtick = alpha * right_encoder_ticks

    delta_left = left_dtick * robot_spec["wheel_radius"]
    delta_right = right_dtick * robot_spec["wheel_radius"]
    delta_theta = (delta_right-delta_left)/robot_spec["wheel_baseline"]
    ...
    v = delta_dist  # replace this with a function that uses the encoder
    w = delta_theta  # replace this with a function that uses the encoder
    ...
    for i in range(belief.shape[0]):
        for j in range(belief.shape[1]):
            ...
            if belief[i, j] > 0:
                ...
                i_new = int((d_t[i, j] - grid_spec['d_min'] + grid_spec["delta_d"] // 2) / grid_spec["delta_d"])
                j_new = int((phi_t[i, j] - grid_spec['phi_min'] + grid_spec["delta_d"] // 2) / grid_spec["delta_phi"])
...
def generate_measurement_likelihood(segments, road_spec, grid_spec):
    ...
    for segment in segments:
        ...
        i = int((d_i - grid_spec['d_min'] + grid_spec["delta_d"] // 2) / grid_spec["delta_d"])
        j = int((phi_i - grid_spec['phi_min'] + grid_spec["delta_d"] // 2) / grid_spec["delta_phi"])
